...
from configuration.paths import TRAINING_DIR
...
def compute_metrics_for_label(segmentation_predicted: np.ndarray, segmentation_reference: np.ndarray,
    ...
    if gt_empty and pred_empty:
        ...
        # 4维输入
        # total_sds = 0
        # for ct_slice in range(len(mask_gt)):
        ...
        # sds = total_sds/len(mask_gt)
    return dice, sds


def compute_disc_for_slice(segmentation_predicted: np.ndarray, segmentation_reference: np.ndarray,
...
if __name__ == "__main__":
    path = r"D:\study\dataset\kits21-master\kits21\data\case_00000\aggregated_MAJ_seg.nii.gz"
    # path1 = r"D:\study\dataset\kits21-master\kits21\data\case_00001\aggregated_MAJ_seg.nii.gz"
    # calculate per metrics
    result = np.zeros((len(HEC_NAME_LIST), 2), dtype=float)
    for i in range(10):
        print(1)
        result += compute_metrics_for_case(path, path)
    print(result)